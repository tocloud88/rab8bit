...
export default function WaterIntakeCalculator() {
  ...
  const handleCalculate = () => {
    ...
    let details: Record<string, any> = {};
      ...
      const weight = n1 || 65;
      const recommendedMl = Math.round(weight * 33);
      const cups = (recommendedMl / 200).toFixed(1);

      computed = '하루 권장 물 섭취량: ' + (recommendedMl / 1000).toFixed(2) + ' L (' + recommendedMl + ' mL)';
      details = {
        '종이컵(200ml) 기준': cups + ' 잔',
        '500ml 생수병 기준': (recommendedMl / 500).toFixed(1) + ' 병',
        '기상 직후': '1잔 (200mL)',
        '식사 30분 전': '각 1잔씩 (총 600mL)'
      };
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="stitch-card p-5 sm:p-7 rounded-3xl space-y-6 border border-indigo-500/25 shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-3 pb-4 border-b border-slate-800">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-black text-white tracking-tight">하루 권장 물 섭취량 계산기</h2>
            <p className="text-xs sm:text-sm text-slate-400 mt-0.5">체중과 일일 활동량, 날씨를 고려하여 하루에 마셔야 할 최적의 음수량(mL)과 시간대별 가이드를 제공합니다.</p>
          </div>
        </div>
