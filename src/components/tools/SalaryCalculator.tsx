...
export default function SalaryCalculator() {
  ...
  const handleCalculate = () => {
    ...
    let details: Record<string, any> = {};
      ...
      const salary = n1 * 10000;
      const nPension = Math.min(salary * 0.045 / 12, 265500);
      const hInsurance = salary * 0.03545 / 12;
      const cInsurance = hInsurance * 0.1295;
      const eInsurance = salary * 0.009 / 12;
      const incomeTax = salary * 0.03 / 12;
      const localTax = incomeTax * 0.1;
      const totalDeduct = nPension + hInsurance + cInsurance + eInsurance + incomeTax + localTax;
      const monthlyNet = Math.round((salary / 12) - totalDeduct);

      computed = '예상 월 실수령액: ' + monthlyNet.toLocaleString() + '원';
      details = {
        '연봉': (salary).toLocaleString() + '원',
        '국민연금 (4.5%)': Math.round(nPension).toLocaleString() + '원',
        '건강보험 (3.545%)': Math.round(hInsurance).toLocaleString() + '원',
        '장기요양 (12.95%)': Math.round(cInsurance).toLocaleString() + '원',
        '고용보험 (0.9%)': Math.round(eInsurance).toLocaleString() + '원',
        '근로소득세': Math.round(incomeTax + localTax).toLocaleString() + '원',
        '월 총 공제액': Math.round(totalDeduct).toLocaleString() + '원'
      };
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="stitch-card p-5 sm:p-7 rounded-3xl space-y-6 border border-indigo-500/25 shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-3 pb-4 border-b border-slate-800">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-black text-white tracking-tight">2026 연봉 실수령액 계산기</h2>
            <p className="text-xs sm:text-sm text-slate-400 mt-0.5">2026년 4대보험 요율과 근로소득세 간이세액표를 반영하여 세후 실수령액과 공제 항목을 실시간 계산합니다.</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">희망 연봉 (만원 단위)</label>
            <input
              type="number"
              placeholder="예: 4500 (4,500만원)"
              value={inputVal1}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">비과세 식대 (월 만원 단위)</label>
            <input
              type="number"
              placeholder="기본 20 (월 20만원)"
              value={inputVal2}
