...
export default function RealtorFeeCalculator() {
  ...
  const handleCalculate = () => {
    ...
    let details: Record<string, any> = {};
      ...
      const dealAmount = n1 * 10000;
      let rate = 0.004;
      if (dealAmount < 50000000) rate = 0.006;
      else if (dealAmount < 200000000) rate = 0.005;
      else if (dealAmount < 900000000) rate = 0.004;
      else rate = 0.005;

      const fee = Math.round(dealAmount * rate);
      const vat = Math.round(fee * 0.1);

      computed = '법정 상한 복비: ' + fee.toLocaleString() + '원 (VAT 별도)';
      details = {
        '거래 금액': dealAmount.toLocaleString() + '원',
        '상한 요율': (rate * 100).toFixed(2) + '%',
        '중개보수': fee.toLocaleString() + '원',
        '부가세(10%)': vat.toLocaleString() + '원',
        '최종 합계': (fee + vat).toLocaleString() + '원'
      };
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="stitch-card p-5 sm:p-7 rounded-3xl space-y-6 border border-indigo-500/25 shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-3 pb-4 border-b border-slate-800">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-black text-white tracking-tight">부동산 중개보수(복비) 계산기</h2>
            <p className="text-xs sm:text-sm text-slate-400 mt-0.5">매매, 전세, 월세 거래금액에 따른 법정 상한 중개보수 요율과 부가세를 자동 계산합니다.</p>
          </div>
        </div>
