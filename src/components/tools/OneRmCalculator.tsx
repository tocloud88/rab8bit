...
export default function OneRmCalculator() {
  ...
  const handleCalculate = () => {
    ...
    let details: Record<string, any> = {};
      ...
      const weight = n1;
      const reps = Math.min(n2 || 1, 30);
      const oneRm = Math.round(weight * (1 + reps / 30));

      computed = '예상 1RM: ' + oneRm + ' kg';
      details = {
        '95% (스트렝스 2회)': Math.round(oneRm * 0.95) + ' kg',
        '85% (근비대 5회)': Math.round(oneRm * 0.85) + ' kg',
        '75% (근비대 10회)': Math.round(oneRm * 0.75) + ' kg',
        '65% (지구력 15회)': Math.round(oneRm * 0.65) + ' kg'
      };
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="stitch-card p-5 sm:p-7 rounded-3xl space-y-6 border border-indigo-500/25 shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-3 pb-4 border-b border-slate-800">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-black text-white tracking-tight">1RM 헬스 3대 중량 계산기</h2>
            <p className="text-xs sm:text-sm text-slate-400 mt-0.5">반복 횟수와 중량을 바탕으로 벤치프레스, 스쿼트, 데드리프트의 최대 1회 중량(1RM)과 %별 훈련 무게표를 산출합니다.</p>
          </div>
        </div>
