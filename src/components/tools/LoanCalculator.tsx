...
export default function LoanCalculator() {
  ...
  const handleCalculate = () => {
    ...
    let details: Record<string, any> = {};
      ...
      const principal = n1 * 10000;
      const rateYear = (n2 || 4.5) / 100;
      const months = 36;
      const r = rateYear / 12;
      const monthlyPay = Math.round((principal * r * Math.pow(1 + r, months)) / (Math.pow(1 + r, months) - 1));
      const totalPaid = monthlyPay * months;
      const totalInterest = totalPaid - principal;

      computed = '월 원리금 상환액: ' + monthlyPay.toLocaleString() + '원';
      details = {
        '대출 원금': principal.toLocaleString() + '원',
        '연 이자율': (rateYear * 100).toFixed(2) + '%',
        '상한 개월': months + '개월 (3년)',
        '총 납부 이자': totalInterest.toLocaleString() + '원',
        '총 상환금액': totalPaid.toLocaleString() + '원'
      };
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="stitch-card p-5 sm:p-7 rounded-3xl space-y-6 border border-indigo-500/25 shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-3 pb-4 border-b border-slate-800">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-black text-white tracking-tight">대출 이자 &amp; 상환 계산기</h2>
            <p className="text-xs sm:text-sm text-slate-400 mt-0.5">원리금 균등, 원금 균등, 만기 일시 상환 방식별 월 납입금과 총 이자 부담액을 비교 분석합니다.</p>
          </div>
        </div>
